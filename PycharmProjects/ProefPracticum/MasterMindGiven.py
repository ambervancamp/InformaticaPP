...
def check_combination():
    ...
    if len(guess_combination) < 4:
        tkMessageBox.showerror("Error!", "Please fill in a complete combo before hitting the 'Check' button!")
    else:
...
def check_game_over():
    ...
    if current_move == MAX_NUMBER_OF_MOVES:
        ...
        tkMessageBox.showinfo("Game over", ending_message)
        root.destroy()
...
def color(color_name):
    ...
    if len(guess_combination) == NUMBER_OF_CIRCLES:
        tkMessageBox.showerror("Colors are full", "No more colors allowed, please hit the 'Check' button to check your result")
    else:
